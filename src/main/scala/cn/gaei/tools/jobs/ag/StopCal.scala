...
//      .filter($"vin".equalTo("LMGGN1S54G1002993")).orderBy($"ts")
      .select($"vin",lit("AG").as("vintype"), $"ts", $"bcm_keyst", $"bms_battst", $"bcs_vehspd", $"loc_lon84", $"loc_lat84")
      .filter($"bcm_keyst".isNotNull && $"bcm_keyst" > 0 && $"bcs_vehspd" >0 && $"loc_lon84".isNotNull)
      .withColumn("data", callUDF("stop",collect_list(struct($"ts",$"bcs_vehspd",$"loc_lon84",$"loc_lat84")).over(ws1)))
      .filter($"data.st" >0 )
